	...
	<?php
		...
		if(isset($_POST['cmd']) && $_POST['cmd'] !=""):
			...
			if($company_id>0):
				...
				$admin_subject = "MS企業指名エントリー";   //"MS求人指名エントリー";//$admin_subject;
			else:
				if($order_id>0):
					...
					$admin_subject = "MS求人指名エントリー"; //"MS求人指名エントリー";
				else:
					if($consultant_id>0):
						...
						$admin_subject = $admin_subject; //"MSコンサル指名エントリー";//"MS求人指名エントリー";//$admin_subject;
					else:
				...
				$title="【メインサイト・Mobile】エントリー";
				$entry_check=0;
				if($order_id>0) {$title="【メインサイト・Mobile】求人指名エントリー";}
				if($company_id>0) {$title="【メインサイト・Mobile】企業指名エントリー";}
				if($consultant_id>0) {$title="【メインサイト・Mobile】エントリー";}
            ...
            if($company_id>0):
                ...
                /**********************************************Begin Process*********************************************/
						/*Fixed*/
						if($company_id==210685 || $company_id==1001693):
							$string_group_cst="";
							$consultant_by_id=12;
							$data_consult=Get_Data("consultant", " and `consultant_Status`=0 and `consultant_entry_id`='$consultant_by_id' order by `consultant_Date` DESC limit 1","`consultant_entry_email`,`consultant_Name`");
							$consultant_email_check=$data_consult['consultant_entry_email'];
							$consultant_name_check=$data_consult['consultant_Name'];
							$string_group_cst.="≪".$consultant_name_check."優先エントリー≫ ";
						else:
							//check in interview
							$List_entry_interview = Get_Data("`interview`", " and `status_interview`=0 and `entry_id`='$company_id' order by `id_interview` DESC limit 1", " `consultant_group_id`");
							if($List_entry_interview['cnt']>0):
							
								$consultant_group_id=$List_entry_interview['consultant_group_id'];
								$consultat_string_ex=explode(",",$consultant_group_id);
								$count_consult_string=count($consultat_string_ex);
								$string_group_cst="";
								for($k=0;$k<$count_consult_string;$k++):
										$consultant_by_id=(int)$consultat_string_ex[$k];
										$data_consult=Get_Data("consultant", " and `consultant_Status`=0 and `consultant_entry_id`='$consultant_by_id' order by `consultant_Date` DESC limit 1","`consultant_entry_email`,`consultant_Name`");
										$consultant_email_check=$data_consult['consultant_entry_email'];
										$consultant_name_check=$data_consult['consultant_Name'];
										$string_group_cst.="≪".$consultant_name_check."優先エントリー≫ ";
								...
								//$string_group_cst.="≪".$consultant_name_check."優先エントリー≫ ";
									...
									//checkin slider
									$string_group_cst="";
									$List_entry_interview = Get_Data("`scrol_vender`", " and `scrol_venderStatus`=0 and `company_id`='$company_id' order by `scrol_venderID` DESC limit 1", " `consultant_id`");
									if($List_entry_interview['cnt']>0):
										$consultant_group_id=$List_entry_interview['consultant_id'];
										$consultat_string_ex=explode(",",$consultant_group_id);
										$count_consult_string=count($consultat_string_ex);
										$string_group_cst="";
										for($k=0;$k<$count_consult_string;$k++):
                ...
                /**********************Begin consultant************/
                /*check consultant job*/
                $consultant_email_list="";
                $check_consultant=false;
                $string_group_cst="";
                if($consultant_id>0 || $order_id>0):
                    $body_content_consultant=$body_content;

                    $List_check_job = Get_Data("job", " and `order_id`='$order_id' ");
                    if($List_check_job['cnt']>0):
                        $consultant_email_list=$List_check_job['entry_consultant'];
                        $check_consultant=true;
                    endif;

                    if($check_consultant):
                        $consultat_string=$consultant_email_list;
                    else:
                        $consultat_string=$consultant_id;
                    endif;
                    $consultat_string_ex=explode(",",$consultat_string);
                    $count_consult_string=count($consultat_string_ex);
                    $string_group_cst="";
                    for($k=0;$k<$count_consult_string;$k++):
                        $consultant_by_id=(int)$consultat_string_ex[$k];
						...
						if($data_consult['cnt']>0 && $cmd_sam=='consultant'):
							$string_group_cst="";
						else:
							if($data_consult['cnt']>0):

		                        $consultant_email_check=$data_consult['consultant_entry_email'];
		                        $consultant_name_check=$data_consult['consultant_Name'];
		                        $string_group_cst.="≪".$consultant_name_check."優先エントリー≫ ";
		                        $list_email_uses .=$consultant_email_check.",";
        ...
        $string_rep_cst=$string_group_cst;

		if(!empty($string_rep_cst)): $string_rep_cst=$string_rep_cst."\n ※担当を希望される場合は、本メール(em@kandc.com)にご返信ください。 ";  endif;
