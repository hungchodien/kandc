...
function check_fileupload(fileName) {
 ...
 $(document).ready(function(){
        $('#submit_form').on('submit', function(){
			...
			/*isChecked = $('#radio_end').prop('checked');
			if(isChecked) {
			     $re2 = true;
			} else {
			    $('#radio_end_error').text($errorText);
                $('#radio_end_error').addClass('error');
                $(this).find('#radio_end').addClass('textError');
                $re2 = false;
			}*/
           ...
           // var email2 = $('#text9').val();
			...
			/*if ($('#s0_confirm').val() == '' && $('#s0_confirm').val() == $('#s0').val()) {
                $('#errors0_confirm').text($errorEmail);
                $('#errors0_confirm').addClass('error');
                $(this).find('#s0_confirm').addClass('textError');
                $re3 = false;
            } else {
			...
			if($re_1 == false){
			 ...
			 if ($re0 == false || $re13 == false || $re1==false )
			 {
				scroll_top_id();
				$('#s0').focus();
                return false;
