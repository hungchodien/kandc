	...
	$("#submit-form .confirm-btn").click(function(e){
		...
		$("#submit-form .validate").each(function(){
			/*if( $(this).find('input,select,input[type="radio"]:checked').val() == "" ){
				$('<p class="rq error">'+empty_err+'</p>').insertAfter($(this));
				result = false;
			}*/	
		...
		if ( $('input[name="confirm-email"]').val() == "" ){
			$('<p class="error rq">'+confirmation_err+'</p>').insertAfter($('input[name="confirm-email"]'));
			result = false;
		}
		
		//Confirmation email
		if ( $('input[name="email"]').val() != $('input[name="confirm-email"]').val() && $('input[name="confirm-email"]').val() != "" ){
				$('<p class="rq not-match error">'+confirmation_err+'</p>').insertAfter('input[name="confirm-email"]');
				result = false;
		}
