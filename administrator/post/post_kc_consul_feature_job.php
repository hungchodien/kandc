<?php 
	...
	$id_f=$_GET['id'];
		...
		{
			$List = Get_Data("kc_consul_feature_job", "and kc_consul_feature_job_id = $id_f");
			$op_vis_str = ($List['kc_consul_feature_job_status'] == "0") ? "Public" : "Draft";
			$op_vis = $List['kc_consul_feature_job_status'];
			$title = $List['kc_consul_feature_job_title'];
			
			$content_feature_job=htmlspecialchars_decode($List['kc_consul_feature_job_content']);
			
			$seo_keyword=htmlspecialchars_decode($List['seo_keyword']);
			$seo_description=htmlspecialchars_decode($List['seo_description']);
			$thumbnail_upload_csv=$List['kc_consul_feature_job_csv']; 
			$seo_add_thumb_meta=$List['seo_add_thumb_meta'];
			
			
			
			list($op_front, $op_bottom) = explode(" ", $List['kc_consul_feature_job_date']);
