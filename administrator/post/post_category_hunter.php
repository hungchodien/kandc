<?php 
	...
	$id_f=$_GET['id'];
		...
		{
			$List = Get_Data("category_hunter", "and category_hunter_ID = $id_f");
			$op_vis_str = ($List['category_hunter_Status'] == "0") ? "Public" : "Draft";
			$op_vis = $List['category_hunter_Status'];
			$title = $List['category_hunter_Name'];
			...
			$thumbnail= $List['category_hunter_Thumbnail'];
			list($op_front, $op_bottom) = explode(" ", $List['category_hunter_Date']);
	...
	<div id="content_body" class="fl">
		<div id="post_body" class="clearfix">
			<div id="post_body_content">
            	<div class="clear">
            	 <p class="title_txt">Name Category Hunter eye</p>
				<input id="post_title" type="text" name="category_hunter" value="<?php echo $title;?>" />
                </div>
