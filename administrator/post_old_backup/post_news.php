<?php 
	...
	$id_f=$_GET['id'];
		...
		else
		{
			...
			$List = Get_Data("news", "and news_id = $id_f");
			$op_vis_str = ($List['news_status'] == "0") ? "Public" : "Draft";
			$op_vis = $List['news_status'];
			$title = $List['news_title'];
			
			$news_excerpt=htmlspecialchars_decode($List['news_excerpt']);
			$news_content = htmlspecialchars_decode($List['news_content']);

			$thumbnail=$List['news_thumnail'];
			

			
			list($op_front, $op_bottom) = explode(" ", $List['news_date']);
