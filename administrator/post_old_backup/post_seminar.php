<?php 
	...
	$id_f=$_GET['id'];
		...
		{
			$List = Get_Data("seminar", "and seminarID = $id_f");
			$op_vis_str = ($List['op_vis'] == "0") ? "Public" : "Draft";
			$op_vis = $List['seminarStatus'];
			$title = $List['seminarTitle'];
			$subTitle=$List['seminarSubTilte'];
			$excerpt_seminar  = htmlspecialchars_decode($List['seminarDescription']);
			$content_seminar  = htmlspecialchars_decode($List['seminarContent']);
			
			$thumbnail=$List['seminarThumbnail'];
			
			
			

			
			list($op_front, $op_bottom) = explode(" ", $List['seminar_Date']);
