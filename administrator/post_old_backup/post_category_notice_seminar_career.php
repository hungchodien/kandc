<?php 
	...
	$id_f=$_GET['id'];
		...
		{
			$List = Get_Data("category_notice_seminar_career", "and id_category_notice_seminar_career = $id_f");
			$op_vis_str = ($List['status_category_notice_seminar_career'] == "0") ? "Public" : "Draft";
			$op_vis = $List['status_category_notice_seminar_career'];
			$title = $List['name_category_notice_seminar_career'];
			
			
			list($op_front, $op_bottom) = explode(" ", $List['date_category_notice_seminar_career']);
	...
	<div id="content_body" class="fl">
		<div id="post_body" class="clearfix">
			<div id="post_body_content">
            	 <p class="title_txt">Name Category</p>
				<input id="post_title" type="text" name="name_category_notice_seminar_career" value="<?php echo $title;?>" />
			</div>
