        ...
        変更がある場合は、画面をお戻りになりまして変更を行ってください。</p>
            ...
            <?php
				if(isset($_POST)){ 
					...
					$input_hide="";
                    for($i=0;$i<count($chkbox_select);$i++):
                        $input_hide.='<input type="hidden" name="chkbox_select[]" value="'.$chkbox_select[$i].'" readonly />';
                    endfor;
				}
			?>
            ...
            <form id="entry-form" class="confirm" method="post" action="<?php echo url_root; ?>mailmagazine/send_mail.php">
            		...
            		<table>
                    		<tr>
                                ...
                                <td>
                                		<?php echo $user_email;?>
                                		<input type="hidden" class="confirm-email" name="email" value="<?php echo $user_email;?>" />
							...
							<tr>
										<th><?php echo $subscribe_posts[$i]; ?></th>
										<td>
											<?php if($chkbox_select[$i]=="0" || $chkbox_select[$i]==""){
												echo '<input type="hidden" name="subscribe_posts[]" value="" readonly />';
												echo '配信希望しない';
											}elseif($chkbox_select[$i]=="1"){
												$admin_subject = 'メルマガ希望';
												echo '<input type="hidden" name="subscribe_posts[]" value="'.$subscribe_posts[$i].'" readonly />';
