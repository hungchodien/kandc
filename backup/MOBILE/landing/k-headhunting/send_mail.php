...
?>	
	<?php
		...
		@include('config_entry.php');
			...
			$name_user_send=$_POST['text1']." ".$_POST['text3'];
		...
		//Check validation before send
	if( isset($user_email) && isset($_POST['text1']) && isset($_POST['text3']) && isset($_POST['select1']) && isset($_POST['select2']) && isset($_POST['select3']) &&
			!filter_var($user_email, FILTER_VALIDATE_EMAIL) === false && strlen(trim($_POST['text1'])) > 0 && strlen(trim($_POST['text3'])) > 0 && strlen(trim($_POST['select1'])) > 0 && strlen(trim($_POST['select2'])) > 0 && strlen(trim($_POST['select3'])) > 0  &&
			$_SESSION["send_mail"] == 'true' ):
		...
		//echo $body_content;
			...
			unset($_SESSION['access_confirm_page']);
...
<?php 
		...
		echo "Please fill in full information";	
	endif; //End check vadilation		
