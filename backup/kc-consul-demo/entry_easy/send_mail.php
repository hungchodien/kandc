...
?>	
	<?php
		...
		//	echo "1";
			...
			if($company_id>0 && $company_id != 1014585):
$admin_subject="Consul-Entry (with resume and clientid)";
$body_content="
----------------------------------------------------------------------
- 以下の求人コードへのエントリーです
----------------------------------------------------------------------
...
----------------------------------------------------------------------
";
elseif($company_id == 1014585):
$admin_subject="Consul-Entry (with resume)";
$body_content="
----------------------------------------------------------------------
- 以下の求人コードへのエントリーです
----------------------------------------------------------------------
...
";
				...
				if($order_id >0):
$admin_subject="Consul-Entry (with resume and orderid)";
$body_content="
----------------------------------------------------------------------
- 以下の求人コードへのエントリーです
----------------------------------------------------------------------
...
";				
					...
					if($consultant_id >0):
$admin_subject="Consul-Entry (with resume and consultantid)";
$body_content="
----------------------------------------------------------------------
- 以下の求人コードへのエントリーです
----------------------------------------------------------------------
【指名コンサルタントID】　　　　　　 {$consultant_id}
.
----------------------------------------------------------------------
- 個人プロフィール
----------------------------------------------------------------------
...
";
			...
			$title="";
				$entry_check="";
				if($order_id>0) {
					$entry_check=$order_id;
					$title="【コンサルサイト】求人指名エントリー（添付有）";
				}
				if($company_id>0) {
					$entry_check=$company_id;
					$title="【コンサルサイト】企業指名エントリー（添付有）";
				}
				if($consultant_id>0) {
					$entry_check=$consultant_id;
					$title="";
				}
				if($entry_check>0):
					switch($entry_check):
							case 1014585:
								$title="【コンサルサイト】エントリー（添付有）";
							break;
						default:
					endswitch;
				endif;
		...
		?>
          <script language="javascript">
   		 //window.location.href = 'complete.php?nameEbis=<?php echo $nameEbis ?>&emailEbis=<?php echo $emailEbis; ?>&birthdayEbis=<?php echo $date_birth; ?>&midEbis=<?php echo $entry_ID_userFix; ?>&pidEbis=kc-consul_pre_entry';
