<?php 
	...
	function Get_New_Job($limit)
	{
		...
		`J`.`age_min`,`J`.`age_max`,`J`.`salary_min`,`J`.`salary_max` from `job_category` as `C` join `job` as `J` on `C`.`job_id`=`J`.`id` group by `J`.`order_id`   order by `J`.`listed_timestamp` DESC limit $limit";
		$query=mysql_query($sql);
...
where `C`.`category_id` in ($id) or `D`.`id` in (select `id` from `category` where `parent_id` in ($id)) 
   ...
   order by `J`.`$order` limit $offset,$rowsperpage";
   $query=mysql_query($sql);
   		return $query;
	}
