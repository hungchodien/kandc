...
//error_reporting(E_ALL);
		...
		//	echo "1";
			...
			if($company_id>0):
				...
				if($admin_subject_txt!=""):
				  $admin_subject_txt1=$admin_subject_txt." and ";
				endif;
				$admin_subject = $admin_subject . " (with ".$admin_subject_txt1."clientid)" ;
				...
				if($order_id>0):
					...
					$admin_subject = $admin_subject . " (with ".$admin_subject_txt1."orderid)" ;
				else:
					if($consultant_id>0):
						...
						$admin_subject = $admin_subject . " (with ".$admin_subject_txt1."consultantid)" ;
					else:
						if($admin_subject_txt!=""):
				  			$admin_subject_txt1=$admin_subject_txt;
							$admin_subject = $admin_subject . " (with ".$admin_subject_txt1.")";
						endif;
			...
			for($k=1; $k<23;$k++):
				...
				//$body_content = str_replace("{text1_note}", "{$text1_note}", $body_content);
				...
				if($order_id>0) {
					...
					$title="【コンサルサイト】求人指名エントリー";
				}
				if($company_id>0) {
					$entry_check=$company_id;
					$title="【コンサルサイト】企業指名エントリー";
				}
				if($consultant_id>0) {
